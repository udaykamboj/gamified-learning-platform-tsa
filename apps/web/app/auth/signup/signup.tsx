    ...
    <>
      ...
      {showMessage && (error || success) && (
        ...
        `}>
          ...
          <button
            ...
            className="p-1 rounded-lg hover:bg-foreground/5 transition-colors shrink-0 opacity-60 hover:opacity-100"
          >
            <X size={18} />
          </button>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 md:px-12 lg:px-20">
        <div className="w-full max-w-[420px] py-10">
          {/* Header */}
          <h1 className="text-[28px] md:text-[32px] font-black text-foreground tracking-tight leading-tight">{t('auth.join_organization')}</h1>
          <p className="mt-2 text-foreground/45 text-[15px] font-medium">{t('auth.join_organization_desc')}</p>
          ...
          <div className="mt-8 flex flex-col items-center gap-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-foreground">{session.data?.user?.first_name} {session.data?.user?.last_name}</p>
                <p className="text-sm text-foreground/45">@{session.data?.user?.username}</p>
              </div>
            </div>

            {/* Organization Info */}
            <div className="w-full text-center py-4 bg-neutral-50 rounded-lg border border-neutral-200">
              <p className="text-sm text-foreground/45 mb-1">{t('auth.joining')}</p>
              <p className="font-semibold text-foreground text-lg">{activeOrg?.name}</p>
            </div>
            ...
            {session.data?.user?.email_verified === false ? (
              ...
              <button
                ...
                className="sl-btn sl-btn-primary w-full transition-all disabled:opacity-50 gap-2"
              >
                {isSubmitting ? (
  ...
  return (
    <>
      ...
      <div className="flex-1 flex items-center justify-center px-6 md:px-12 lg:px-20">
        <div className="w-full max-w-[420px] py-10">
          {/* Header */}
          <h1 className="text-[28px] md:text-[32px] font-black text-foreground tracking-tight leading-tight">{t('auth.invite_required')}</h1>
          <p className="mt-2 text-foreground/45 text-[15px] font-medium">{t('auth.invite_required_desc', { org: activeOrg?.name })}</p>
            ...
            <FormLayout onSubmit={validateCode}>
              <FormField name="invite_code">
                <div className="flex items-center space-x-2 mb-1.5">
                  <Form.Label className="grow text-[13px] font-semibold text-foreground/70">{t('auth.invite_code')}</Form.Label>
                </div>
              ...
              <Form.Submit asChild>
                <button
                  ...
                  className="sl-btn sl-btn-primary w-full mt-2 transition-all disabled:opacity-50 gap-2"
                >
                  {isSubmitting ? (
                    <Loader2 size={18} className="animate-spin" />
                  ) : (
