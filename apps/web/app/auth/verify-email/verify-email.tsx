        ...
        >
                ...
                <div className="flex-1 flex items-center justify-center px-6 md:px-12 lg:px-20">
                    <div className="w-full max-w-[420px] py-10">
                        {/* Header */}
                        <h1 className="text-[28px] md:text-[32px] font-black text-foreground tracking-tight leading-tight text-center">{t('auth.verify_email_title')}</h1>

                        {/* Loading State */}
                        {isVerifying && (
                            <div className="mt-8 flex flex-col items-center gap-4">
                                <Loader2 className="h-12 w-12 animate-spin text-foreground/40" />
                                <p className="text-foreground/45 text-sm font-medium">{t('auth.verifying_email')}</p>
                            </div>
                        )}
                        ...
                        {!isVerifying && error && (
                            <div className="mt-8 space-y-5">
                                ...
                                <p className="text-sm text-foreground/45 text-center font-medium">
                                    {t('auth.verification_trouble')}
                                </p>
                                <Link
                                    href="/login"
                                    className="sl-btn sl-btn-primary w-full transition-all"
                                >
                        ...
                        {!isVerifying && success && (
                            <div className="mt-8 space-y-5">
                                <div className="flex flex-col items-center gap-4 text-center">
                                    ...
                                    <div>
                                        <h2 className="font-bold text-lg text-foreground">{t('auth.email_verified_success')}</h2>
                                        <p className="text-sm text-foreground/45 font-medium mt-1">{t('auth.email_verified_message')}</p>
                                    </div>
                                </div>
                                <Link
                                    href="/dashboard"
