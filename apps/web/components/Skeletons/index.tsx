        ...
        <div className="pt-1.5 flex items-center justify-between border-t border-gray-100">
          ...
          <div className="flex items-center gap-2">
            <div className="flex -space-x-2">
              <div className="w-5 h-5 rounded-full bg-gray-200 animate-pulse border-2 border-white" />
              <div className="w-5 h-5 rounded-full bg-gray-200 animate-pulse border-2 border-white" />
            </div>
...
export function CourseDetailSkeleton() {
  return (
    <div className="flex flex-col gap-6 w-full max-w-4xl mx-auto px-4 py-8">
      {/* Hero */}
      <div className="rounded-xl overflow-hidden bg-gray-200 animate-pulse aspect-video w-full" />
      ...
      <div className="space-y-3 mt-2">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="flex flex-col gap-2 p-4 rounded-xl border border-gray-100 bg-card">
            {/* Chapter heading */}
...
export function ActivitySkeleton() {
  return (
    <div className="flex flex-col w-full h-full min-h-screen gap-4 p-6">
      ...
      {/* Editor body */}
      <div className="flex-1 rounded-xl bg-card border border-gray-100 p-6 space-y-4">
        <div className="h-6 w-1/2 bg-gray-200 rounded animate-pulse" />
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="h-4 bg-gray-200 rounded animate-pulse" style={{ width: `${85 - i * 5}%` }} />
        ))}
        <div className="h-40 w-full bg-gray-200 rounded-xl animate-pulse mt-4" />
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="h-4 bg-gray-200 rounded animate-pulse" style={{ width: `${75 + i * 5}%` }} />
        ))}
      </div>
    </div>
  )
...
export function CommunityListSkeleton() {
  return (
    <div className="flex flex-col gap-3 w-full">
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} className="flex items-center gap-4 p-4 rounded-xl bg-card border border-gray-100">
          {/* Icon / avatar */}
          ...
          {/* CTA pill */}
          <div className="h-7 w-16 rounded-full bg-gray-200 animate-pulse flex-shrink-0" />
        </div>
      ))}
    </div>
  )
  ...
  return (
    <div className="flex flex-col gap-6 p-6 w-full">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="rounded-xl bg-card border border-gray-100 p-4 flex flex-col gap-3">
      ...
      <div className="rounded-xl bg-card border border-gray-100 p-6 flex flex-col gap-4">
        <div className="h-4 w-1/4 bg-gray-200 rounded animate-pulse" />
        <div className="h-48 w-full bg-gray-200 rounded-xl animate-pulse" />
      </div>

      {/* List section */}
      <div className="rounded-xl bg-card border border-gray-100 p-6 flex flex-col gap-3">
        <div className="h-4 w-1/4 bg-gray-200 rounded animate-pulse mb-2" />
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-gray-200 animate-pulse flex-shrink-0" />
            <div className="h-3 flex-1 bg-gray-200 rounded animate-pulse" />
            <div className="h-3 w-12 bg-gray-200 rounded animate-pulse flex-shrink-0" />
          </div>
        ))}
      </div>
    </div>
...
export function PageSkeleton() {
  return (
    <div className="flex flex-col min-h-screen w-full">
      {/* Nav bar */}
      <div className="h-14 border-b border-gray-100 bg-card flex items-center px-6 gap-4">
      ...
      <div className="flex-1 p-8 flex flex-col gap-6 max-w-5xl mx-auto w-full">
        ...
        {/* Content blocks */}
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="rounded-xl bg-card border border-gray-100 p-6 space-y-3">
