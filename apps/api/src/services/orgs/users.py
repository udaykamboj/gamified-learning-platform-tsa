...
from src.security.rbac.constants import ADMIN_ROLE_ID, PLATFORM_ROLE_IDS
...
async def get_organization_users(
    ...
    if not await is_user_superadmin(acting_user_id, db_session):
        ...
        if not await is_org_admin(acting_user_id, org.id, db_session):
            raise HTTPException(
                status_code=403,
                detail="Only administrators can view organization members",
            )
...
async def export_organization_users_csv(
    ...
    if not await is_user_superadmin(export_acting_user_id, db_session):
        ...
        if not await is_org_admin(export_acting_user_id, org.id, db_session):
            raise HTTPException(
                status_code=403,
                detail="Only administrators can export organization members",
            )
...
async def update_user_role(
    ...
    role_id = role.id
    # Accounts are either Admin or Student. There is no teacher/custom role to
    # assign (docs/refactor/progress/00-requirements.md, R2).
    if role_id not in PLATFORM_ROLE_IDS:
        raise HTTPException(
            status_code=400,
            detail="Accounts can only be Admin or Student",
        )
...
async def get_list_of_invited_users(
    ...
    if not await is_user_superadmin(acting_user_id, db_session):
        ...
        if not await is_org_admin(acting_user_id, org.id, db_session):
            raise HTTPException(
                status_code=403,
                detail="Only administrators can view organization invites",
            )
