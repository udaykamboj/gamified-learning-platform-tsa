"""Compact authoring DSL for catalog activity bodies, compiled to ProseMirror.
...
The catalog is synced in place on every boot and compared against the files, so
random quiz ids would rewrite every activity in the database on every sync.
"""
...
# hard error at sync time rather than a blank activity.
_PREFIX_NODES = {
    "h2": ("heading", 2),
    "h3": ("heading", 3),
    "p": ("paragraph", None),
...
class ContentError(ValueError):
    """The catalog authored a block this module cannot compile."""
