...
class AIConfig(BaseModel):
    ...
    # STARLAB_AI_IMAGE_MODEL; defaults to the GA `gemini-2.5-flash-image`
    ...
    # is Google) or `gemini_api_key`. Override the exact model id with
    # STARLAB_AI_TTS_MODEL; defaults to `gemini-2.5-flash-preview-tts`. All
...
class MailingConfig(BaseModel):
    ...
    system_email_sender_name: Optional[str] = "StarLab"
...
def _env_bool(env_value, yaml_value):
    ...
    string "false" — which is truthy. So `STARLAB_SELF_HOSTED=false` used to
    read as True and pin tenancy to "single", collapsing the CORS regex and
    making the session cookie host-only; `STARLAB_SSL=false` produced https
...
def get_starlab_config() -> StarLabConfig:
    ...
    # Development Mode
    env_development_mode_str = os.environ.get("STARLAB_DEVELOPMENT_MODE", "None")
    ...
    # Sentry config
    env_sentry_dsn = os.environ.get("STARLAB_SENTRY_DSN")
    sentry_dsn = env_sentry_dsn or yaml_config.get("general", {}).get("sentry_dsn")

    # Environment (dev or prod)
    starlab_env = os.environ.get("STARLAB_ENV", "dev")

    # SaaS Mode (enables plan-based gating and usage limits)
    env_saas_mode = os.environ.get("STARLAB_SAAS", "None")
    saas_mode = (
    ...
    # Security Config
    env_auth_jwt_secret_key = os.environ.get("STARLAB_AUTH_JWT_SECRET_KEY")
    ...
    if not auth_jwt_secret_key:
        raise ValueError(
            "SECURITY ERROR: STARLAB_AUTH_JWT_SECRET_KEY must be set. "
            "Generate a secure key with: python -c \"import secrets; print(secrets.token_urlsafe(32))\""
        )
    if len(auth_jwt_secret_key) < 32:
        raise ValueError(
            "SECURITY ERROR: STARLAB_AUTH_JWT_SECRET_KEY must be at least 32 characters. "
    ...
    env_site_name = os.environ.get("STARLAB_SITE_NAME")
    env_site_description = os.environ.get("STARLAB_SITE_DESCRIPTION")
    env_contact_email = os.environ.get("STARLAB_CONTACT_EMAIL")
    env_domain = os.environ.get("STARLAB_DOMAIN")
    os.environ.get("STARLAB_PORT")
    env_ssl = os.environ.get("STARLAB_SSL")
    env_port = os.environ.get("STARLAB_PORT")
    env_use_default_org = os.environ.get("STARLAB_USE_DEFAULT_ORG")
    env_allowed_origins = os.environ.get("STARLAB_ALLOWED_ORIGINS")
    env_cookie_domain = os.environ.get("STARLAB_COOKIE_DOMAIN")
    env_frontend_domain = os.environ.get("STARLAB_FRONTEND_DOMAIN")
    env_tenancy = os.environ.get("STARLAB_TENANCY")

    # Allowed origins should be a comma separated string
    if env_allowed_origins:
        env_allowed_origins = env_allowed_origins.split(",")
    env_allowed_regexp = os.environ.get("STARLAB_ALLOWED_REGEXP")
    env_self_hosted = os.environ.get("STARLAB_SELF_HOSTED")
    env_sql_connection_string = os.environ.get("STARLAB_SQL_CONNECTION_STRING")
    ...
    #   - "multi":  slug.{STARLAB_DOMAIN} subdomain detection. Requires EE
    ...
    #     `acme.starlab.io` to org `acme`).
    #   - Anything else → "single".
    # Existing operators upgrading do not need to set STARLAB_TENANCY for
    ...
    if tenancy_explicit:
        ...
        #   1. SaaS mode is on (STARLAB_SAAS=true) — SaaS deployments are
        #      always multi-tenant by definition.
        #   2. EE is available AND a shared-subdomain cookie was configured
        #      (STARLAB_COOKIE_DOMAIN starts with "."). The dotted cookie
    ...
    if tenancy not in ("multi", "single"):
        raise ValueError(
            f"STARLAB_TENANCY must be 'multi' or 'single' (got {tenancy_raw!r})."
        )
    ...
    # the operator opts in with STARLAB_COOKIE_DOMAIN_ALLOW_BROAD=true.
    ...
    if (
        ...
        allow_broad = os.environ.get(
            "STARLAB_COOKIE_DOMAIN_ALLOW_BROAD", ""
        ).lower() in ("true", "1", "yes")
        if "." not in tenant_eTLD:
            raise ValueError(
                f"SECURITY ERROR: STARLAB_COOKIE_DOMAIN={cookies_domain!r} is "
        ...
        if not allow_broad:
            ...
            _cfg_log.getLogger(__name__).warning(
                "STARLAB_COOKIE_DOMAIN is set to the broad parent %r. "
                "Every subdomain will share the session cookie. If that is "
                "intentional, set STARLAB_COOKIE_DOMAIN_ALLOW_BROAD=true to "
    ...
    env_content_delivery_type = os.environ.get("STARLAB_CONTENT_DELIVERY_TYPE")
    content_delivery_type: str = env_content_delivery_type or (
        (yaml_config.get("hosting_config", {}).get("content_delivery", {}).get("type"))
        or "filesystem"
    )  # default to filesystem

    env_bucket_name = os.environ.get("STARLAB_S3_API_BUCKET_NAME")
    env_endpoint_url = os.environ.get("STARLAB_S3_API_ENDPOINT_URL")
    bucket_name = (
    ...
    env_gemini_api_key = os.environ.get("STARLAB_GEMINI_API_KEY")
    env_is_ai_enabled_str = os.environ.get("STARLAB_IS_AI_ENABLED")

    gemini_api_key = env_gemini_api_key or yaml_ai_config.get("gemini_api_key")
    ai_image_model = os.environ.get("STARLAB_AI_IMAGE_MODEL") or yaml_ai_config.get("image_model")
    ai_tts_model = os.environ.get("STARLAB_AI_TTS_MODEL") or yaml_ai_config.get("tts_model")

    # Provider-agnostic generation settings (env takes precedence over yaml).
    ai_provider = os.environ.get("STARLAB_AI_PROVIDER") or yaml_ai_config.get("provider")
    ai_api_key = os.environ.get("STARLAB_AI_API_KEY") or yaml_ai_config.get("api_key")
    ai_base_url = os.environ.get("STARLAB_AI_BASE_URL") or yaml_ai_config.get("base_url")
    ai_model_fast = os.environ.get("STARLAB_AI_MODEL_FAST") or yaml_ai_config.get("model_fast")
    ai_model_standard = os.environ.get("STARLAB_AI_MODEL_STANDARD") or yaml_ai_config.get("model_standard")
    ai_model_pro = os.environ.get("STARLAB_AI_MODEL_PRO") or yaml_ai_config.get("model_pro")
    ai_embedding_provider = os.environ.get("STARLAB_AI_EMBEDDING_PROVIDER") or yaml_ai_config.get("embedding_provider")
    ai_embedding_model = os.environ.get("STARLAB_AI_EMBEDDING_MODEL") or yaml_ai_config.get("embedding_model")
    _ai_embedding_dims = os.environ.get("STARLAB_AI_EMBEDDING_DIMENSIONS") or yaml_ai_config.get("embedding_dimensions")
    ...
    # Redis config
    env_redis_connection_string = os.environ.get("STARLAB_REDIS_CONNECTION_STRING")
    redis_connection_string = env_redis_connection_string or yaml_config.get(
        "redis_config", {}
    ).get("redis_connection_string")

    # Mailing config
    env_email_provider = os.environ.get("STARLAB_EMAIL_PROVIDER")
    env_resend_api_key = os.environ.get("STARLAB_RESEND_API_KEY")
    env_system_email_address = os.environ.get("STARLAB_SYSTEM_EMAIL_ADDRESS")
    env_system_email_sender_name = os.environ.get("STARLAB_SYSTEM_EMAIL_SENDER_NAME")
    env_smtp_host = os.environ.get("STARLAB_SMTP_HOST")
    env_smtp_port = os.environ.get("STARLAB_SMTP_PORT")
    env_smtp_username = os.environ.get("STARLAB_SMTP_USERNAME")
    env_smtp_password = os.environ.get("STARLAB_SMTP_PASSWORD")
    env_smtp_use_tls = os.environ.get("STARLAB_SMTP_USE_TLS")
    ...
    # Defaults to "StarLab" so an unset deployment keeps the historical
    # From name exactly as it was.
    system_email_sender_name = env_system_email_sender_name or yaml_config.get(
        "mailing_config", {}
    ).get("system_email_sender_name", "StarLab")
    ...
    env_tinybird_api_url = os.environ.get("STARLAB_TINYBIRD_API_URL")
    env_tinybird_ingest_token = os.environ.get("STARLAB_TINYBIRD_INGEST_TOKEN")
    env_tinybird_read_token = os.environ.get("STARLAB_TINYBIRD_READ_TOKEN")
    ...
    env_judge0_api_url = os.environ.get("STARLAB_JUDGE0_API_URL")
    env_judge0_client_id = os.environ.get("STARLAB_JUDGE0_CLIENT_ID")
    env_judge0_client_secret = os.environ.get("STARLAB_JUDGE0_CLIENT_SECRET")
    ...
    # Payments config
    env_stripe_secret_key = os.environ.get("STARLAB_STRIPE_SECRET_KEY")
    env_stripe_publishable_key = os.environ.get("STARLAB_STRIPE_PUBLISHABLE_KEY")
    env_stripe_webhook_standard_secret = os.environ.get("STARLAB_STRIPE_WEBHOOK_STANDARD_SECRET")
    env_stripe_webhook_connect_secret = os.environ.get("STARLAB_STRIPE_WEBHOOK_CONNECT_SECRET")
    env_stripe_client_id = os.environ.get("STARLAB_STRIPE_CLIENT_ID")
    ...
    if not _TESTING_TENANCY:
        ...
        if tenancy == "multi":
            ...
            # parent for browsers in production, so `STARLAB_DOMAIN=localhost`
            # is rejected here.
            try:
            ...
            if not (ee_available or saas_mode):
                raise ValueError(
                    "STARLAB_TENANCY=multi requires the Enterprise Edition "
                    "or STARLAB_SAAS=true. Either install the `ee/` folder, "
                    "unset STARLAB_DISABLE_EE, or use STARLAB_TENANCY=single."
                )
            if not domain or "localhost" in str(domain).lower():
                raise ValueError(
                    "STARLAB_TENANCY=multi requires STARLAB_DOMAIN to be set "
                    "to a routable parent domain (e.g. 'starlab.io'). "
                    f"Got domain={domain!r}."
                )

        if tenancy == "single":
            # In single mode the cookie domain is always host-only — operators
            # who set STARLAB_COOKIE_DOMAIN expecting cross-subdomain auth
            # will be surprised. Warn loudly.
            if env_cookie_domain:
                _t_logger.warning(
                    "STARLAB_COOKIE_DOMAIN=%r is set but STARLAB_TENANCY=single "
                    "ignores it (cookies are host-only in single mode).",
                    env_cookie_domain,
                )

        # Deprecations: both flags are subsumed by STARLAB_TENANCY.
        if env_use_default_org is not None:
            _t_logger.warning(
                "STARLAB_USE_DEFAULT_ORG is deprecated; tenancy mode "
                "(STARLAB_TENANCY=%s) now controls default-org behavior.",
                tenancy,
            )
        if env_self_hosted is not None:
            _t_logger.warning(
                "STARLAB_SELF_HOSTED is deprecated; use STARLAB_TENANCY=single "
    ...
    if not development_mode and saas_mode and not _LOGGED_MISSING_INTERNAL_KEYS:
        ...
        if not os.environ.get("STARLAB_PLATFORM_API_KEY"):
            missing.append(
                "STARLAB_PLATFORM_API_KEY (pack activation, active-user overage billing)"
            )
        if not (
            os.environ.get("STARLAB_GOOGLE_OAUTH_CLIENT_ID")
            or os.environ.get("STARLAB_GOOGLE_CLIENT_ID")
        ):
            missing.append(
                "STARLAB_GOOGLE_OAUTH_CLIENT_ID (Google OAuth audience "
    ...
    # Create StarLabConfig object
    config = StarLabConfig(
        site_name=site_name,
