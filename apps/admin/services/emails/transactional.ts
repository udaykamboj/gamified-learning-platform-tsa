...
export async function sendWelcomeAccountMail(args: { email: string; username?: string }): Promise<void> {
  ...
  await send(email, 'Welcome to StarLab 👋', {
    accentColor: '#171717',
    heading: 'Welcome to StarLab!',
    subtitle: username
    ...
    cta: { label: 'Get started', href: 'https://www.starlab.io/home' },
  })
}
...
export async function sendContactMail(args: {
  ...
  await send(to || 'hello@starlab.app', `New contact form message from ${name || fromEmail}`, {
    accentColor: '#171717',
