...
const BACKEND_URL = (process.env.NEXT_PUBLIC_STARLAB_BACKEND_URL || 'http://localhost:1338').replace(/\/+$/, '')

// Cookie names (must match the API routes)
const ACCESS_TOKEN_COOKIE = 'LH_admin_access'
const REFRESH_TOKEN_COOKIE = 'LH_admin_refresh'
...
export async function getServerSession(): Promise<Session | null> {
  try {
    ...
    if (accessToken?.value) {
      ...
      const sessionResponse = await fetch(`${BACKEND_URL}/api/v1/admin/users/session`, {
        method: 'GET',
        headers: {
