...
async function proxyRequest(
  ...
  let pathSegments = request.nextUrl.pathname.replace('/api/auth/', '')
  const search = request.nextUrl.search

  // In apps/admin, all auth is administrator auth
  if (pathSegments === 'login') pathSegments = 'admin/login'
  if (pathSegments === 'refresh') pathSegments = 'admin/refresh'

  // Map to backend URL: /api/auth/login -> /api/v1/auth/admin/login
  ...
  const reqAccessTokenCookie = ADMIN_ACCESS_TOKEN_COOKIE
  const reqRefreshTokenCookie = ADMIN_REFRESH_TOKEN_COOKIE
