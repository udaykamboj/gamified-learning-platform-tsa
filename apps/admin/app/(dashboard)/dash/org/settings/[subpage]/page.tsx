...
const getSettingTabs = (t: any): TabConfig[] => [
  ...
  { id: 'ai', label: t('dashboard.organization.settings.tabs.ai') || 'AI', customIcon: '/starlab_ai_simple_colored.png', requiredPlan: 'standard' },
  ...
  const tabs: DashTabItem[] = SETTING_TABS.map((tab) => ({
    ...
    icon: tab.customIcon
      ...
      : tab.icon
        ? <tab.icon size={16} />
        : null,
    href: getUriWithOrg((params?.orgslug || 'default'), '') + `/dash/org/settings/${tab.id}`,
