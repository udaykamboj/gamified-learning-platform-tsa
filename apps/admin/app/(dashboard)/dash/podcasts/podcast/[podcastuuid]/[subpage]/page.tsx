        ...
        >
          <div>
            {params.subpage === 'general' && (
              <EditPodcastGeneral orgslug={(params?.orgslug || 'default')} />
            )}
            {params.subpage === 'content' && (
              <EditPodcastEpisodes orgslug={(params?.orgslug || 'default')} podcastuuid={podcastuuid} />
            )}
            {params.subpage === 'distribution' && (
              <PodcastDistribution orgslug={(params?.orgslug || 'default')} podcastuuid={podcastuuid} />
            )}
          </div>
        </motion.div>
      </PodcastProvider>
    ...
    <div className="ps-4 pe-4 sm:ps-10 sm:pe-10 text-sm tracking-tight bg-[#fcfbfc] z-10 nice-shadow relative">
      <div className="pt-6 pb-4">
        ...
        <div className="flex items-center justify-between mt-4">
          <div className="flex items-center space-x-4">
            <Link
              href={getUriWithOrg((params?.orgslug || 'default'), '/dash/podcasts')}
          ...
          {podcast && (
            <Link
              href={getUriWithOrg((params?.orgslug || 'default'), `/podcast/${params.podcastuuid}`)}
              target="_blank"
