...
description: Build your own headless learning platform on the web with Next.js and the StarLab API. Start with an anonymous public catalogue, then add authentication, enrollment, and progress tracking.
---
...
StarLab is **API-first**. The official web app is just one client of a public REST API — which means you can build your *own* learning platform on top of the same API, with your own design, your own routing, and your own brand.

This guide takes you from an empty folder to a working **headless learning platform**: a Next.js app that lists your courses, plays your content, signs users in, enrolls them, and tracks their progress — all powered by the StarLab API. It's the kind of result you'll find in the [`starlab/headless-examples`](https://github.com/starlab/headless-examples) repository.
...
<Tabs items={['Stage 1 — Anonymous', 'Stage 2 — Authenticated']}>
  ...
  <Tabs.Tab>
    ...
    - **Signup & login** with the StarLab auth endpoints.
...
Your Next.js app is a thin, fast client in front of the StarLab API. Reads happen in **Server Components**; anything involving a token goes through a **Route Handler acting as a Backend-for-Frontend (BFF)**, so StarLab tokens never reach the browser. Media is streamed straight from the API's content-delivery endpoint.

<HeadlessArchitecture />

<Callout type="info">
Using a server-side BFF isn't just for security — it also sidesteps CORS entirely. Your browser only ever talks to your own Next.js origin, and your server talks to StarLab.
</Callout>

## The resources you'll touch

Everything in StarLab hangs off an **Organization**. Content is a tree of **Course → Chapter → Activity**. A learner's progress is a separate tree: each **User** has a **Trail**, which holds a **Run** per enrolled course, which holds a **Step** per completed activity.
...
- **A running StarLab instance.** Spin one up locally with the [Quickstart](/getting-started/quickstart) or [self-hosting guide](/self-hosting), or use [StarLab Cloud](https://starlab.app). The examples assume the API is at `http://localhost:1338`.
