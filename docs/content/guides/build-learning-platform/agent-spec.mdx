---
title: Agent Spec
description: A self-contained, copy-paste specification a coding agent can follow to build a headless StarLab learning platform with Next.js, including the exact API contract and acceptance criteria.
---
...
Copy everything in the block below, fill in the **Configuration** values at the top, and paste it into your coding agent (see [Let an Agent Build It](/guides/build-learning-platform/with-an-agent) for the workflow). It's written *for the model* and contains the real StarLab API contract, so the agent builds against the API that actually exists.
...
````markdown
# SPEC: Build a headless learning platform on the StarLab API

## Configuration (fill these in before starting)
- API_BASE = http://localhost:1338/api/v1     # your StarLab API base URL
...
that reads courses and content from the StarLab REST API and supports user
...
## Hard rules
- StarLab access/refresh tokens MUST NEVER reach the browser. Do all
...
- src/lib/starlab.ts   : API_BASE/ORG_SLUG consts, `lhFetch` (public reads,
...
## Environment
Create .env.local:
  NEXT_PUBLIC_STARLAB_API_URL=<API_BASE>
  NEXT_PUBLIC_STARLAB_ORG_SLUG=<ORG_SLUG>

## Build order
1. Scaffold: npx create-next-app@latest . --typescript --app --tailwind --eslint --src-dir --use-npm
2. Stage 1: lib/starlab.ts -> catalogue -> course page -> activity viewer.
