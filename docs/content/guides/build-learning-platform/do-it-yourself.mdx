...
description: A complete, tested, step-by-step guide to building a headless learning platform with Next.js and the StarLab API — public catalogue first, then authentication, enrollment, and progress tracking.
---
...
This is the full build. By the end you'll have a Next.js app that browses your courses, plays their content, signs learners in, enrolls them, and tracks progress — all on top of the StarLab API.
...
<Callout type="info">
**Prerequisites** (from the [overview](/guides/build-learning-platform)): Node 18+, a running StarLab instance at `http://localhost:1338`, your **org slug**, and at least one **public, published course** with content. Everything reads the API base from one env var, so adjust it if your instance is elsewhere.
</Callout>
...
NEXT_PUBLIC_STARLAB_API_URL=http://localhost:1338/api/v1
NEXT_PUBLIC_STARLAB_ORG_SLUG=your-org-slug
```
...
Every call to StarLab goes through one small helper. Create `src/lib/starlab.ts`. This mirrors the pattern the official frontend uses in `apps/web/services/utils/ts/requests.ts` — a thin `fetch` wrapper that surfaces the API's `{ "detail": "..." }` error shape.

```ts filename="src/lib/starlab.ts"
export const API_URL = process.env.NEXT_PUBLIC_STARLAB_API_URL!
export const ORG_SLUG = process.env.NEXT_PUBLIC_STARLAB_ORG_SLUG!

/**
 * Fetch JSON from the StarLab API.
...
export async function lhFetch<T = any>(
  ...
  if (!res.ok) {
    ...
    throw new Error(`StarLab API ${res.status}: ${detail}`)
  }
  return res.json()
}
...
```ts filename="src/lib/starlab.ts (continued)"
export type Course = {
  course_uuid: string
  name: string
  description: string
...
import { lhFetch, courseThumbnail, ORG_SLUG, type Course } from '@/lib/starlab'
...
Open [http://localhost:3000](http://localhost:3000) and you'll see your courses. (If StarLab is also on `:3000`, run the API on its usual port and Next will pick `:3001` — or set `PORT`.)
...
import Link from 'next/link'
import { lhFetch, type CourseMeta } from '@/lib/starlab'
...
import { lhFetch, activityMedia, ORG_SLUG, type Activity } from '@/lib/starlab'
...
Now we add accounts. The golden rule: **StarLab tokens never reach the browser.** Your Next.js app logs in on the server, stores the tokens in its *own* httpOnly cookie, and attaches them when calling the API. This is the Backend-for-Frontend (BFF) pattern.
...
Create `src/lib/session.ts` to read and write the session cookie. It stores the StarLab tokens, encrypted-at-rest concerns aside, in an httpOnly cookie that the browser can't read from JavaScript.
...
import { API_URL } from '@/lib/starlab'
...
import { NextRequest, NextResponse } from 'next/server'
import { API_URL, lhFetch, ORG_SLUG } from '@/lib/starlab'
...
export async function authedFetch<T = any>(
  ...
  if (!res.ok) throw new Error(`StarLab API ${res.status}`)
  return res.json()
}
```
...
- **Compare with the reference** — see [`starlab/headless-examples`](https://github.com/starlab/headless-examples) for a complete project.
