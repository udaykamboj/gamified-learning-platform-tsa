---
title: Webhooks
description: Subscribe to StarLab events, verify HMAC-SHA256 signatures, and react to learning activity in real time with a Next.js webhook receiver.
---
...
Webhooks let StarLab **push events to your application** the moment they happen — a learner enrolls, completes a course, submits an assignment, signs up — so you can react without polling. Use them to sync a CRM, send custom emails, update a dashboard, or kick off any workflow.
...
2. When a subscribed event fires, StarLab sends a `POST` to your URL with a JSON body, **signed** with a per-endpoint secret.
3. Your receiver **verifies the signature**, then handles the event.
4. If your endpoint doesn't return a `2xx`, StarLab **retries** — up to 3 attempts, waiting 1s then 4s between them — and logs every delivery.
...
curl -X POST "http://localhost:1338/api/v1/orgs/1/webhooks" \
  ...
  -d '{
    "url": "https://your-app.com/api/webhooks/starlab",
...
```ts filename="src/app/api/webhooks/starlab/route.ts"
import { NextRequest, NextResponse } from 'next/server'
import crypto from 'node:crypto'

const SECRET = process.env.STARLAB_WEBHOOK_SECRET! // the whsec_… value
...
export async function POST(req: NextRequest) {
  ...
  // 4. Return 2xx quickly so StarLab marks the delivery successful.
...
- **Outbound requests are SSRF-protected**: StarLab validates the target URL and the resolved IP before every delivery, so endpoints must be publicly reachable — `localhost` and private/reserved IP ranges are always rejected (in every environment).
