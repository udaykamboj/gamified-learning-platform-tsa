...
- **Infrastructure** (Docker): PostgreSQL with pgvector + Redis run in containers via `.starlab/docker-compose.dev.yml`
...
```bash
docker compose -f .starlab/docker-compose.dev.yml -p starlab-dev down
```
