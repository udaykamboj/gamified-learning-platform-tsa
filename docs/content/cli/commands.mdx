...
Full reference for all StarLab CLI commands.
...
Show the live status of each StarLab container.
...
Update StarLab to the latest release or a specific version. See [Updates](/self-hosting/maintenance/updates) for details.
...
npx starlab update --to 1.4.2
# or using the short form:
npx starlab update -v 1.4.2
```

| Flag | Description |
|---|---|
| `-v, --to <version>` | Pull a specific image tag from `ghcr.io/starlab/app` (e.g. `1.4.2`, `v1.4.2`). Defaults to `latest`. |
...
If the migration step fails, the CLI prints the rollback command and exits with a non-zero code. Your pre-upgrade backup is in `{installDir}/backups/`. To roll back: restore the backup, revert the image tag in `docker-compose.yml`, and run `npx starlab start`.
</Callout>
...
Backups are saved to `{installDir}/backups/starlab-backup-{timestamp}.tar.gz`.
...
```bash
npx starlab restore <archive.tar.gz>
```

Alternatively:

```bash
npx starlab backup --restore <archive.tar.gz>
```

<Callout type="warning">
Restore only works with local (Docker) databases, not external Postgres connections. Restart services after restoring: `npx starlab stop && npx starlab start`.
</Callout>
...
```bash
npx starlab deployments
```

Features:
- List all StarLab deployments with container status
- Scale memory limits for `starlab-app`, `db`, and `redis`
...
Run `npx starlab doctor` — it scans logs and surfaces the root cause. Common causes:
...
If `npx starlab update` reports a migration failure:

```bash
# 1. Restore your pre-upgrade backup
npx starlab restore backups/db-pre-upgrade-*.sql.gz

# 2. Revert the image tag in docker-compose.yml to the previous version
# (open the file and change image: ghcr.io/starlab/app:<new> back to <old>)

# 3. Restart on the old image
npx starlab start
```

Then check the [StarLab releases page](https://github.com/starlab/starlab/releases) and the [GitHub issues](https://github.com/starlab/starlab/issues) for your version before retrying.

### Database unreachable after update

The `starlab-db` container may need a moment after startup. Wait 20-30 seconds and run `npx starlab health`. If health checks keep failing, run `npx starlab logs` to see the Postgres logs.
...
docker compose exec db psql -U starlab -c "DROP DATABASE starlab; CREATE DATABASE starlab;"
npx starlab restore <backup.tar.gz>
```
