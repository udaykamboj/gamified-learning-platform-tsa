...
The setup wizard (`npx starlab@latest setup`) is an interactive 6-step process that generates a complete Docker Compose deployment. You can go back and edit any step before confirming.
...
- **HTTPS options** (non-localhost only):
  ...
  - **Manual SSL** — You handle the reverse proxy or Cloudflare. StarLab runs on HTTP internally.
...
| --- | --- | --- |
| `starlab-app-{id}` | `ghcr.io/starlab/app:latest` (or `:dev`) | Next.js + FastAPI application |
| `starlab-db-{id}` | `pgvector/pgvector:pg16` | PostgreSQL database |
| `starlab-redis-{id}` | `redis:7.2.3-alpine` | Redis cache |
| `starlab-caddy-{id}` | `caddy:2-alpine` | Reverse proxy with auto-SSL |
| `starlab-nginx-{id}` | `nginx:alpine` | Reverse proxy (no auto-SSL) |
...
- **Stable** — Recommended for production (pulls `ghcr.io/starlab/app:latest`)
- **Dev** — Bleeding edge, latest features (pulls `ghcr.io/starlab/app:dev`)

To pin to a specific image tag after setup, use `starlab update --version <tag>`.
