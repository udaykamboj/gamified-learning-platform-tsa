...
StarLab supports two storage backends for uploaded content such as images, videos, and documents.
...
Content is persisted in a Docker volume, so it survives container restarts. The `starlab backup` command includes filesystem content in its backups.
...
STARLAB_S3_API_BUCKET_NAME=starlab-content
STARLAB_S3_API_ENDPOINT_URL=https://s3.amazonaws.com
```
...
| Backup | Included in `starlab backup` | Managed separately by your storage provider |
...
If you want S3-compatible storage without relying on a cloud provider, you can run MinIO alongside StarLab. MinIO provides an S3-compatible API on your own infrastructure.
...
STARLAB_CONTENT_DELIVERY_TYPE=s3api
STARLAB_S3_API_BUCKET_NAME=starlab-content
STARLAB_S3_API_ENDPOINT_URL=http://minio:9000
```
