...
Keeping StarLab up to date ensures you have the latest features, bug fixes, and security patches.
...
# https://github.com/starlab/starlab/releases
...
StarLab uses [Alembic](https://alembic.sqlalchemy.org/) to evolve the database schema between versions. After the new container starts, the CLI prompts:
...
```bash
docker compose exec starlab-app sh -c "cd /app/api && uv run alembic upgrade head"
```
</Callout>

### Installs Created Before v1.3

Early StarLab installs created their schema via SQLAlchemy's `create_all` without Alembic tracking. The update command detects this automatically and stamps the current schema as the baseline before running any delta migrations — so `upgrade head` applies only new changes instead of replaying everything.
...
npx starlab restore backups/db-pre-upgrade-<timestamp>.sql.gz

# Revert the image tag in docker-compose.yml to the previous version
# (edit the file: change image: ghcr.io/starlab/app:<new> back to <old>)

# Restart on the old image
npx starlab start
```

Backups are named `db-pre-upgrade-{timestamp}.sql.gz` when created by `update`, and `starlab-backup-{timestamp}.tar.gz` when created by the `backup` command. The pre-upgrade backup created by `update` includes only the database dump; to also preserve your `.env`, run `npx starlab backup` manually before updating.

<Callout type="warning">
Uploaded files stored on the filesystem (in the `starlab_content_*` Docker volume) are **not** included in the database backup. To back up uploaded files, use `docker volume export` or a volume-level snapshot provided by your hosting platform.
</Callout>
...
Review the [StarLab release notes](https://github.com/starlab/starlab/releases) before updating to understand what has changed. Some updates may include:
