...
The StarLab backend is a **FastAPI** application written in Python, located at `apps/api` in the project. It serves as the central API layer that handles all business logic and data management.
...
<Callout type="info">
Swagger UI (`/docs/`) and ReDoc (`/redoc/`) are only accessible when `STARLAB_DEVELOPMENT_MODE=True` is set. They are disabled in production.
</Callout>
...
The backend runs on **port 1338** by default during local development (configurable via `STARLAB_PORT`). In production Docker deployments, it runs on port 9000. Install dependencies and run the server:
...
When using the StarLab CLI (`starlab dev`), the backend starts automatically with hot reload enabled.
