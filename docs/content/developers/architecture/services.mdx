...
StarLab relies on several supporting services that run alongside the main applications.

- In **development** (`starlab dev`), PostgreSQL and Redis run in Docker via `.starlab/docker-compose.dev.yml` while the API, web, and collab processes run locally on the host.
- In **self-hosted production**, the StarLab CLI generates a `docker-compose.yml` from templates in `apps/cli/src/templates/` that runs PostgreSQL, Redis, a single `starlab-app` container (which bundles web + API + collab), and either nginx or Caddy as a reverse proxy.

## PostgreSQL

`pgvector/pgvector:pg16` (PostgreSQL 16 with the pgvector extension) is the primary database for StarLab. It stores all persistent application data including:
...
Both route traffic to the single `starlab-app` container, which internally serves the Next.js frontend, the FastAPI backend, and the Hocuspocus collaboration server.
