...
This guide walks through the full sequence of API calls needed to build a course from scratch -- the same calls the StarLab web UI makes when an editor clicks through the dashboard. Use it as a reference when writing your own migration script.
...
```bash
#!/usr/bin/env bash
# build-demo-course.sh — create a complete course end-to-end via the StarLab API.
set -euo pipefail
...
COURSE=$(curl -fsS -X POST "$API/courses/?org_id=$ORG_ID" \
  ...
  -F 'about=Created end-to-end by the StarLab migration quickstart script.' \
  -F 'public=true' \
  -F 'thumbnail_type=image' \
...
- **Hosted MP4 sample** in the StarLab video player.
