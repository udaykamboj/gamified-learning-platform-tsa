...
<Callout type="info">
The built-in task types — `QUIZ`, `FORM`, `CODE`, `SHORT_ANSWER`, `NUMBER_ANSWER` — are auto-graded by StarLab against their `contents` schema. `CUSTOM` (and `FILE_SUBMISSION`) are **not** auto-graded and are graded manually.
</Callout>

## Submissions and grading

Learners are referenced by their existing StarLab `user_id`. These endpoints let you read submissions, write a learner's answer on their behalf, and finalize grades.
...
To write a learner's answer, pass their StarLab id as `on_behalf_of_user_id`. The learner must already be a member of the token's organization.
